...
def walkThroughDataset(dataSetDir):
    ...
    for index, imageFile in tqdm(enumerate(listOfFiles)):
        ...
        dst_path = '/media/stevemaary/新加卷/data/src/VPGNet/scene_4'
        if not ops.exists(dst_path):
            os.makedirs(dst_path)
        dst_path = dst_path + str(index) + '.jpg'
        cv2.imwrite(dst_path, img_bgr)

        cv2.imshow('VPGNet Dataset Quick Inspector', img_bgr)
